...
/**
 * 「导入」按钮 + 弹窗:运营上传直播明细表(所有主播画像表,.csv / .xlsx / .xls),
 * 走 /api/operator/data/live/import,按 (平台, UID, 日期) upsert 到 LiveStat。
 * 只入库开播时长>0 的记录;成功后 router.refresh()。
 */
...
export default function ImportLiveButton() {
  ...
  async function submit() {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/operator/data/live/import", {
        method: "POST",
        body: fd,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>导入直播明细表</DialogTitle>
          <DialogDescription>
            上传「所有主播画像表」(.csv / .xlsx),按 平台 + UID + 日期 覆盖更新。
            只入库开播时长&gt;0 的记录;游戏流水/主播收入两列不入库。
          </DialogDescription>
        </DialogHeader>
        ...
        {result ? (
          <div className="space-y-3 py-2">
            ...
            <div className="rounded-md border border-border p-3 text-sm">
              ...
              <div className="mt-1">
                入库 <strong className="tabular-nums">{result.rowCount}</strong> 条直播记录
              </div>
              {result.skippedCount > 0 && (
                <div className="mt-1 text-xs text-muted-foreground">
                  跳过(开播时长≤0):
                  <strong className="tabular-nums">{result.skippedCount}</strong> 条
                </div>
              )}
            </div>
          </div>
